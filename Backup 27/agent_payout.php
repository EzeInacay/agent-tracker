...
$sql = "SELECT request_id, request_date, amount, mode, provider, details, remarks, status, approval_date 
        FROM payout_requests $where ORDER BY request_date DESC";
...
<form method="GET" action="" class="history-search-form" 
    ...
    <select name="status_filter">
        ...
        <option value="Declined"  <?= (($_GET['status_filter'] ?? '') === 'Declined') ? 'selected' : '' ?>>Declined</option>
    </select>

    <button type="submit" class="btn">Search</button>
    <a href="agent_payout.php" class="btn btn-secondary">Clear</a>
</form>
    ...
    <div class="table-container" role="region" aria-label="Payout Request History" tabindex="0">
        <table class="history-table">
            ...
            <?php if (!empty($history)): ?>
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <tr><td colspan="8" style="text-align:center;">No payout requests found</td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
...
<div id="payoutModal" class="modal-overlay" style="display:none;">
    <div class="modal-content">
        ...
        <form action="request_payout.php" method="POST" class="payout-form" onsubmit="return openConfirmPayout(event)">
...
<div id="confirmPayoutModal" class="modal-overlay" style="display:none;">
  <div class="modal-content">
    <h4>Confirm Payout Request</h4>
    <p>Are you sure you want to submit this payout request?</p>
    <div class="modal-buttons">
      <button onclick="confirmPayout()" class="btn btn-success">Yes, Submit</button>
      <button onclick="closeConfirmPayout()" class="btn btn-secondary">Cancel</button>
    </div>
  </div>
</div>
...
<script>
    ...
    let payoutForm; // reference to form

function openConfirmPayout(e) {
    e.preventDefault();
    // validate amount first
    if (!validateAmount()) return false;
    payoutForm = e.target;
    document.getElementById('confirmPayoutModal').style.display = 'flex';
    return false; // stop normal submit
}

function closeConfirmPayout() {
    document.getElementById('confirmPayoutModal').style.display = 'none';
}

function confirmPayout() {
    if (payoutForm) {
        payoutForm.submit(); // finally submit form
    }
}
