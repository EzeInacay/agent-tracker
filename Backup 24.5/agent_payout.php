...
// 📌 Weekly earnings
$sql = "SELECT SUM((b.final_price - b.ratehawk_price) * ?) as weekly
...
$stmt = $conn->prepare($sql);
$stmt->bind_param("di", $commissionDecimal, $agentId);
$stmt->execute();
$weeklyEarnings = $stmt->get_result()->fetch_assoc()['weekly'] ?? 0;

// 📌 Gross commissions (all-time)
$sql = "SELECT SUM((b.final_price - b.ratehawk_price) * ?) as gross
        FROM booking_status bs
        INNER JOIN bookings b ON bs.booking_id = b.booking_id
        WHERE b.agent_id=? 
        AND bs.booking_status IN ('Confirmed','Completed')";
...
<!-- NAVIGATION -->
<div class="navbar">
    <div>Welcome, <?= htmlspecialchars($agentName) ?></div>
    <div class="nav-links">
