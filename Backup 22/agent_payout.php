...
async function api(url, opts={}) {
  ...
  // If PHP accidentally outputs whitespace, guard json()
...
async function checkNotifications(initial=false) {
  ...
  if (data && data.success && Number(data.unseen) > 0) {
    ...
    // If we're already ON the dashboard, auto-mark as seen once.
    if (initial && location.pathname.endsWith('agent_payout.php')) {
      await markSeen(); // hide immediately and persist in DB
    }
  } else {
    notifDot.style.display = 'none';
  }
...
/* poll & also when tab becomes active */
...
/* Clicking Dashboard should mark and then go */
...
/* your existing modal + form JS can stay as-is below */
